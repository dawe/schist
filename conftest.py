...
scs.inference.fit_model(adata, nested=True , neighbors_key='foo', n_samples=2)
...
print("PLANTED MODEL")
scs.inference.fit_model(adata, assortative=True, use_weights=False, save_model='foo', n_samples=2)
lineages = scs.tl.cr_lineages(adata, model_key='ppbm', level=0)
print("SB MODEL")
scs.inference.fit_model(adata, nested=False, use_weights=False, save_model='foo', n_samples=2)
...
scs.inference.fit_model_multi([d1, d2], nested=True, n_samples=2)
u = scs._utils.get_multi_graph_from_adata([d1, d2])
print(u.num_vertices())
mdata = MuData({'A':d1, 'B':d2})
scs.inference.fit_model_multi(mdata, nested=False, n_samples=2)
